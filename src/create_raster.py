    ...
    with rasterio.open(raster_path) as src:
        ...
        with rasterio.open(temp_output, 'w', **meta) as dst:
            for i in range(1, src.count + 1):
                ...
                empty_band = np.full((meta['height'], meta['width']), nodata_value, dtype=src.dtypes[0])
            ...
            for i in range(1, src.count + 1):
                data = src.read(i)
                dst.write(data, i, window=Window(col_offset, row_offset, orig_width, orig_height))
...
def warp_image_with_gcp(temp_output, output_path, coords):
    with rasterio.open(temp_output, 'r+') as src:
        ...
        if nodata is not None:
            data_mask = np.any(data != nodata, axis=0)  # Avoid using the name 'mask' for variables
        else:
        ...
        gcps = [
            GroundControlPoint(col=top_left[1], row=top_left[0], x=coords[0][0], y=coords[0][1]),  # top-left
            GroundControlPoint(col=top_right[1], row=top_right[0], x=coords[1][0], y=coords[1][1]),  # top-right
            GroundControlPoint(col=bottom_right[1], row=bottom_right[0], x=coords[2][0], y=coords[2][1]),  # bottom-right
            GroundControlPoint(col=bottom_left[1], row=bottom_left[0], x=coords[3][0], y=coords[3][1])  # bottom-left
        ]

        transform = from_gcps(gcps)
        src.transform = transform
        polygon = Polygon(coords)
        gdf = gpd.GeoDataFrame([{'geometry': polygon}], crs="EPSG:4326")  # Ensure CRS matches
        nodata_value = 255
        # Use rio_mask instead of mask to avoid conflict
        out_image, out_transform = rio_mask(src, [gdf.geometry.iloc[0].__geo_interface__], nodata=nodata_value, crop=True)
