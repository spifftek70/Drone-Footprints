...
def calculate_fov(
    ...
    cal_pitch_rad = np.radians(
        gimbal_pitch_deg % 90
    )  # Direct conversion to radians without modulo operation
    cal_yaw_rad = np.radians(
        (gimbal_yaw_deg + 90) * -1
    )  # Direct conversion to radians with adding 90

    # Calculate bounding polygon using camera orientation
    bbox = camera_calculator.getBoundingPolygon(
        FOVh, FOVv, altitude, cal_roll_rad, cal_pitch_rad, cal_yaw_rad
    )
