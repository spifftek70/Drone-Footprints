...
from Utils.logger_config import get_logger, TqdmWebSocket
...
def process_metadata(metadata, indir_path, geotiff_dir, sensor_dimensions):
    ...
    try:
        ...
        line_coordinates = []
        outer = TqdmWebSocket(total=len(metadata), position=0, desc='Image Files', leave=False)
        pbar = TqdmWebSocket(total=len(metadata), position=1, leave=False, bar_format='{desc}')
        line_feature = ""
